...
var start = function(injector, config, launcher, globalEmitter, preprocess, fileList, webServer,
  ...
  // The single run browser test manager.
  var singleRunBrowsers = new BrowserCollection(globalEmitter);
    ...
    socket.on('register', function(info) {
      ...
      if (newBrowser) {
        ...
        if (config.singleRun) {
          ...
          if (++started === config.browsers.length) {
            singleRunBrowsers.invokeExecute(config);
          }
        }
      }
