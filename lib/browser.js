...
var Browser = function(id, fullName, /* capturedBrowsers */ collection, emitter, socket, timer,
  ...
  var activeSocketsIds = function() {
    return _.map(activeSockets, 'id').join(', ');
  };
  ...
  this.reconnect = function(newSocket) {
    if (this.state === states.EXECUTING_DISCONNECTED) {
      this.state = states.EXECUTING;
      log.debug('Reconnected on %s.', newSocket.id);
    } else if (this.state === states.EXECUTING || this.state === states.READY) {
      log.debug('New connection %s (already have %s)', newSocket.id, activeSocketsIds());
    } else if (this.state === states.DISCONNECTED) {
      this.state = states.READY;
  ...
  this.execute = function(config) {
    _.invoke(activeSockets, 'emit', 'execute', config);
...
Browser.STATE_READY = states.READY;
Browser.STATE_EXECUTING = states.EXECUTING;
Browser.STATE_READY_DISCONNECTED = states.READY_DISCONNECTED;
Browser.STATE_EXECUTING_DISCONNECTED = states.EXECUTING_DISCONNECTED;
Browser.STATE_DISCONNECTED = states.DISCONNECTED;
